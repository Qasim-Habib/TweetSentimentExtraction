...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for ei in range(input_length):
      encoder_output, encoder_hidden,embedded = encoder(
...
for pair in validation_set:
  validation_pairs.append(tensorsFromPair(pair))
def validate(encoder, decoder, max_length=MAX_LENGTH):
  loss=0
  criterion = nn.NLLLoss()
  with torch.no_grad():
    for iter in range(len(validation_pairs)):
        validation_pair = validation_pairs[iter]
        ...
        # Without teacher forcing: use its own predictions as the next input
          for di in range(target_length):
              ...
              if decoder_input.item() == EOS_token:
                  break
        temp_loss=temp_loss/target_length
        loss+=temp_loss
  print('the loss of validation set is:',loss.item() / len(validation_pairs))
  return loss.item() / len(validation_pairs)
...
def evaluate(encoder, decoder, max_length=MAX_LENGTH):
  ...
  for i,test_input in enumerate(test):
    ...
    with torch.no_grad():
      ...
      encoder_outputs = torch.zeros(max_length, encoder.hidden_size, device=device)
      for ei in range(input_length):
            encoder_output, encoder_hidden,embedded = encoder(input_tensor[ei],
                                                     encoder_hidden)
            encoder_outputs[ei] += encoder_output[0, 0]
      ...
      last_word=output_sentence.split(' ')
	  last_word=last_word[len(last_word)-1]
      if last_word=='negative' or last_word=='neutral' or last_word=='positive':
        classification_test.append(last_word)
      else:
        classification_test.append('difficulttweet')
      output_test.append(output_sentence)
  return output_test,classification_test
  
def trainIters(encoder, decoder, learning_rate=0.01,print_every=1000,epochs=20):
  print_loss_total = 0
  ...
  for e in range(epochs):
    ...
    for iter in range(len(training_pairs)):
      if iter==index_begin:
        temp=time.time() - start_training
        totalTimeTraining+=temp
        loss_validate=validate(encoder, decoder)
        list_lossValidation.append(loss_validate)
        start_training=time.time() 
    ...
    loss_every_epoch/=len(training_pairs)
    list_lossTraining.append(loss_every_epoch)
    list_epochs.append(e)
  return list_lossTraining,list_epochs,list_lossValidation,totalTimeTraining
hidden_size = 256
encoder1 = EncoderRNN(input_lang.n_words, hidden_size).to(device)
attn_decoder1 = AttnDecoderRNN(hidden_size, input_lang.n_words, dropout_p=0.1).to(device)
size = 0
for par in encoder1.parameters():
  calc = 1
  lis = list(par.data.size())
  for i in range(0, len(lis)):
    calc = calc * lis[i]
  size += calc
for par in attn_decoder1.parameters():
  calc = 1
  lis = list(par.data.size())
  for i in range(0, len(lis)):
    calc = calc * lis[i]
  size += calc
print('the number of parameters is: ',size)
list_lossTraining,list_epochs,list_lossValidation,totalTimeTraining=trainIters(encoder1, attn_decoder1)
print("training time is:", totalTimeTraining)
output_test,classification_test=evaluate(encoder1, attn_decoder1)
for i,out in enumerate(output_test):
   hist_words=[]
   s=''
   for word in out.split(' '):
     if word not in hist_words and word!='negative' and word!='neutral' and word!='positive':
       s+=word
       s+=' '
   ...
   if len(s)==0:
     s=test[i][1]
   output_test[i]=s
sub_df['selected_text']=output_test
sub_df['text']=test_df['text']
sub_df['predicit_sentiment']=classification_test
sub_df['actual_sentiment']=test_df['sentiment']
fig = plt.figure()
plt.plot(list_epochs,list_lossTraining, color='blue')
plt.plot(list_epochs, list_lossValidation, color='red')
plt.legend(['Train Loss', 'Validation Loss'], loc='upper right')
plt.xlabel('number of epochs')
plt.ylabel(' loss')
fig
sub_df.to_csv('df.csv')
